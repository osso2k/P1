...
const upload = () => {
    ...
    const handleAnalyze = async ({companyName,jobTitle,jobDescription,file}:{companyName:string , jobTitle:string,jobDescription:string,file:File})=>{
        ...
        if(!uploadedFile) return setStatusText("Error faaailed to upload file")
        
        setStatusText("Converting to image")
        const imageFile= await convertPdfToImage(file)

        if(!imageFile.file) return setStatusText("Error faaailed to Convert to Image")
        
        setStatusText("Uploading the image")
        const uploadedImage = await fs.upload([imageFile.file])

        if(!uploadedImage) return setStatusText("Error faaailed to upload imagee")
